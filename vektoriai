...
void mediana(std::vector<int> pazymiai)
{
    int counter = 0;
...
void automated (duomenys Eil[], int i, int how_many_marks)
{
    Eil[i].egz = random_paz();
    for (int x = 0; x < how_many_marks; x++)
    {
        Eil[i].paz.push_back(random_paz());
    }
    Eil[i].galut = Eil[i].galut / how_many_marks;
...
void ranka(duomenys Eil[], int i)
{
    ...
    do {
        ...
        if (Eil[i].paz.at(counter) != -1) { Eil[i].galut = Eil[i].galut + (float)Eil[i].paz.at(counter); }
        counter++;
...
void rezultatai (duomenys Eil[], int studentu_sk)
{
    std::cout << "\n\n";
